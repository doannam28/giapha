...
var SEO = {
    ...
    generate_slug: function (title, ele) {
        ...
        // Loại bỏ tất cả ký tự không phải chữ cái, số, hoặc dấu gạch ngang
...
var AJAX_CRUD_MODAL = {
    ...
    close: function () {
        $('#modal_form').on('hidden.bs.modal', function (e) {
            ...
            $(this).find('.preview img').attr('src', '/public/default-thumbnail.webp');
    ...
    add: function () {
        ...
        $('#modal_form').find('.btnSaveCopy').addClass('none');
        $('#modal_form').find('.modal-footer .btn-primary').removeClass('none').removeClass('btnSaveCopy').addClass('btnSave').text('Save');
        $('.btnSaveUpdate').addClass('none');
        return false;
    },
    edit: function (func) {
        slug_disable = true;
            ...
            success: function (data) {
                ...
                if (data.type === "warning") {
                    $.each(data.validation, function (i, val) {
                        let input = $('[name="' + i + '"]');
                        let value = '<bug>' + val + '</bug>';
                        if (input.parent().hasClass('input-group')) {
                            input.closest('.input-group').after(value);
                        } else {
                            input.after(value);
                        }
    ...
    save_update: function () {
        let modal_form = $('#modal_form');
        let url;
        modal_form.find('.btnSaveUpdate').attr('disabled', true);
        url = url_ajax_update;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                modal_form.find('.btnSaveUpdate').attr('disabled', false);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                console.log(jqXHR);
                console.log(textStatus);
    ...
    init: function () {
        ...
        doc.on('click', '.btnSaveUpdate', function (e) {
            e.preventDefault();
            AJAX_CRUD_MODAL.save_update();
        });
...
var FUNC = {
    ...
    imgError: (image) => {
        image.onerror = "";
        image.src = base_url + 'public/default-thumbnail.webp';
        return true;
    },
    getImageThumb: (thumbnail) => {
        // Kiểm tra nếu thumbnail có chứa "http"
        if (thumbnail.startsWith('http')) {
            return thumbnail; // Trả về thumbnail nếu là URL
        }
        // Nếu không, kết hợp với media_url
        ...
        moxman.browse({
            ...
            oninsert: function (args) {
                $.each(args['files'], function (i, value) {
                    count = count + 1;
                    var url = value.url;
                    var urlImageResponse = url.replace('/media', '');
    ...
    itemGallery: function (count, urlImageResponse) {
        let getImageThumb = FUNC.getImageThumb(urlImageResponse);
        let html = "";
        html += '<li>';
        html += '<div class="imgs item_gallery item_' + count + '" data-count="' + count + '">';
        html += '<img class="img_gallery" src="' + getImageThumb + '" id="item_' + count + '" data-count="' + count + '">';
...
$(document).ready(function () {
    ...
    $(".m-datatable__table").scroll(function () {
        $(".wrapper1")
            .scrollLeft($(".m-datatable__table").scrollLeft());
    });
