...
class Page extends Public_Controller
{
    ...
    {
        $oneItem = $this->_page->getByField('slug', $slug);
        $view = 'page';
        if (empty($oneItem)) {
            $oneItem = $this->_category->getByField('slug', $slug);
            $this->_category->_recursive_child_id($this->_all_category, $oneItem->id);
            ...
            $paging['base_url'] = get_url_category_post($oneItem, 1);
        ...
        if (empty($oneItem)) {
            ...
            $view = 'post';
            $data['category'] = $oneCategory = $this->_post->getByIdCategoryPost($oneItem->id);
            $this->_category->_recursive_child_id($this->_all_category, !empty($oneCategory->id) ? $oneCategory->id : '');
            $listCateId = $this->_category->_list_category_child_id;
            $data['list_post'] = $this->_post->getDataPostByCategory($listCateId, $page, 10);
            $data['post_newst'] = $this->_post->getDataPost();

            $this->breadcrumbs->push('Trang chủ', base_url());
            $this->breadcrumbs->push($oneCategory->title, get_url_category_post($oneCategory));
        ...
        $data['main_content'] = $this->load->view($this->template_path . $view . '/index' . $layoutView, $data, TRUE);
    ...
    public function bao_gia($slug, $page = 1)
    {
        ...
        $data['main_content'] = $this->load->view($this->template_path . 'page/page_bao_gia', $data, TRUE);
        $this->load->view($this->template_main, $data);
    }
