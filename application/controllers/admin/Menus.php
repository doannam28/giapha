...
class Menus extends Admin_Controller
{
    ...
    public function ajax_load_city($location = 12)
    {
        $data = $this->_data->search(['location_id' => $location]);
        $output = [];
    ...
    public function ajax_save_menu()
    {
        ...
        if ($menuLocation == 12) {
            ...
            foreach ($response as $key => $block) {
                if (!empty($block['id'])) $tempID[] = $block['id'];
            };
            $datanotin = $this->_data->getDBFE([
                'location_id' => $menuLocation,
                'not_in' => $tempID
            ]);
            if (!empty($datanotin)) foreach ($datanotin as $key => $delete) {
                $this->_data->delete(['id' => $delete['id']]);
            };
        ...
        if (is_array($response)) {
            $topmenusorder = 1;
            if (!empty($response)) foreach ($response as $key => $block) {
                // if (!empty($block['id'])) $tmp['id'] = trim($block['id']);
                $tmp['title'] = trim($block['label']);
                $tmp['class'] = trim($block['cls']);
                $tmp['data_id'] = trim($block['value']);
                $tmp['link'] = trim($block['link']);
    ...
    public function getListMenu($type) {
        $list = $this->_data->get_list_by_type($type);
        return $list;
    }
}
