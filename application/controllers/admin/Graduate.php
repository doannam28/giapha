...
class Graduate extends Admin_Controller
{
    protected $_data;
    protected $_user, $_graduate;

    public function __construct()
    {
        parent::__construct();
        //tải thư viện
        $this->load->model(['Family_model', 'users_model', 'Graduate_model']);
    ...
    {
        $data['heading_title'] = "Quản lý bảng vàng";
        $data['heading_description'] = "Danh sách bảng vàng";
    ...
    public function ajax_list()
    {
        ...
        // Lấy toàn bộ dữ liệu
        $offset = ($page - 1) * $limit;
        $listData = $this->_graduate->getDataGraduate($limit, $offset);
        $listAll  = $this->_graduate->getDataGraduate(10000000000, 0);
        $matchedItems = [];
        foreach ($listData as $key => $itemGraduate) {
            $itemGraduate = (object)$itemGraduate;
            // Kiểm tra id của $listAll với person_id của $listGraduate
            $matchedItems[] = [
                'checkID' => $itemGraduate->gid,
                'id' => $offset + $key + 1,
                'person_name' => $itemGraduate->person_name,
                'father_name' => $itemGraduate->father_name,
                'year' => $itemGraduate->year,
                'university' => $itemGraduate->university,
            ];
        }

        // Trả về danh sách các item trùng khớp (nếu cần)
    ...
    {
        $term = $this->input->get("q"); // Lấy giá trị từ query string 'q'
        $list = $this->_data->get_data(); // Lấy danh sách dữ liệu từ model
        $array_data = array();

        foreach ($list as $user) {
            if (empty($term) || stripos($user->full_name, $term) !== false) {
                $user = (object) $user;
                $array_data[] = [
                    'id' => $user->id,
                    'text' => $user->full_name
        ...
        $this->returnJson($array_data); // Trả về kết quả dưới dạng JSON
    }
    ...
    public function ajax_add()
    {
        ...
        if ($id = $this->_graduate->save($data)) {
    ...
    public function ajax_update_field()
    {
        ...
        $response = $this->_graduate->update(['person_id' => $id], [$field => $value]);
        if ($response != false) {
            $message['type'] = 'success';
            $message['message'] = "Cập nhật thành công !";
            $this->delete_cache_all();
