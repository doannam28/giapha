...
class Menus_model extends STEVEN_Model
{
    ...
    public function getDBFE($args)
    {
        extract($args);
        $this->db->select('*');
        $this->db->from($this->table);
        if (!empty($location_id)) $this->db->where('location_id', $location_id);
        if (!empty($not_in)) $this->db->where_not_in('id', $not_in);
        $query = $this->db->get();
...
public function get_list_by_type($type){
             $this->db->where('location_id', $type);
            $this->db->from($this->table);
            $data = $this->db->get()->result();
            return $data;
}
